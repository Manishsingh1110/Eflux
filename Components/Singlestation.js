    ...
    return (
        <View>
            {Station ? (Station.map((item) => {
                return (
                    <ScrollView className="h-[92vh] bg-slate-100" key={item._id}>
                        ...
                        <View className="flex flex-wrap flex-row pb-5">
                            {
                                item.Slot.map((slot) => {
                                    return (
                                        <View className="w-1/3 mt-2 h-20 p-2" key={slot._id}>
                                            <View className="bg-teal-700 h-20 rounded-lg justify-center p-2" >
                                                <Text className="text-center text-white mb-2">  Slot | {slot.Slotname}</Text>
                                                <Text className="text-center text-white">Port | {slot.Connectortype}</Text>
                                            </View>
                                        </View>
                                    )
                                })
                        ...
                        <View>
                            <Text className="text-lg text-teal-900  px-5">Available Time</Text>
                        </View>
                        <View className="flex flex-wrap flex-row pb-5">
                            {
                                item.Timeslot.map((slot) => {
                                    return (
                                        <View className="w-1/3 mt-2 h-20 p-2" key={slot._id}>
                                            <View className="bg-teal-700 h-20 rounded-lg justify-center p-2">
                                                <Text className="text-center text-white mb-2"> Time</Text>
                                                <Text className="text-center text-white">{slot.Duration}</Text>
                                            </View>
                                        </View>
                                    )
                                })
                        ...
                        <View className="flex flex-wrap flex-row justify-between px-7 pb-4 mt-5">
                            <View>
                                <Text className="text-xl  text-teal-900 mt-1"><Ionicons name='card-outline' size={25}></Ionicons> Price  {item.Priceperunit}/unit</Text>
                            </View>
                            <View className="bg-teal-700 w-20 h-10 rounded-lg justify-center p-1" style={{ "backgroundColor": "#033E3E" }}>
                                <Link underlayColor="#ffffff00" to={`/bookstation/${item._id}`}>
