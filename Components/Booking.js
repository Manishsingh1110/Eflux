...
const CurrentBooking = (props) => {
  const [booking, setBookings] = React.useState({savedstationbooking:[],savedbatterybooking:[]})
    ...
    <View style={styles.container}>
      ...
      <Text style={styles.text}>Charging Slots</Text>
        <View className="flex flex-row flex-wrap">
          {
            station ? (
              booking.savedstationbooking.map((slot) => {
                return (
                  <View style={styles.card} className="w-1/2" key={slot._id}>
                    ...
                    <Text className="pb-1 h-16 overflow-hidden"> <Ionicons name='time' />{slot.Station[0].Stationaddress}</Text>
